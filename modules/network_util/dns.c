...
static uint8 dns_parse_respond(DNS_CLIENT *dns, uint8 *resIP)
{
    ...
    if (recvLen == 0)
    {
        OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DNS recv none");
        return DNS_NONE;
    }
...
static bool dns_try_to_resend(DNS_CLIENT *dns, uint8 *sendBuf, uint16 sendLen)
{
    ...
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DNS retry[%d], cur[%d] last[%d]", dns->retryCnt, curTime, dns->lastTime);
    ...
    if (dns->retryCnt < DNS_MAX_RETRY) 
    {
        ...
        if ((curTime - dns->lastTime) > DNS_WAIT_TIME) 
        {
            ...
            OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DNS timeout, resend");
        }
    }
    else
    {
...
uint8 dns_process(DNS_CLIENT *dns, char *domain, uint8 *resIP)
{
    ...
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DNS SERVER %d.%d.%d.%d",
    ...
    dns->lastTime = os_get_second_count();
    ...
    while (1)
    {
        ...
        {
            OS_DBG_ERR(DBG_MOD_NET, "DNS FAILED, TIMEOUT");
            return DNS_ERR;
        }
...
void dns_client_init(DNS_CLIENT *dns)
{
    ...
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DNS Init OK");
}
