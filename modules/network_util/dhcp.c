...
static void dhcp_state_update(DHCP_CLIENT *dhcp, uint8 state)
{
    ...
    //reset counter
    dhcp->retryCnt = 0;
    dhcp->lastTime = os_get_second_count();
}
...
static void dhcp_send_discover(DHCP_CLIENT *dhcp)
{
    ...
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP SEND DISCOVER to %d.%d.%d.%d:%d",
...
static void dhcp_send_request(DHCP_CLIENT *dhcp)
{
    ...
    dhcp->sendto(dhcp->sock, (uint8*)&ripMsg, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP SEND REQUEST to %d.%d.%d.%d:%d",
                             ip[0], ip[1], ip[2], ip[3], DHCP_SERVER_PORT);
}
...
static void dhcp_send_release_decline(DHCP_CLIENT *dhcp, bool release)
{
    ...
    dhcp->sendto(dhcp->sock, (uint8*)&ripMsg, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP SEND RELEASE to %d.%d.%d.%d:%d",
                            ip[0], ip[1], ip[2], ip[3], DHCP_SERVER_PORT);
...
static uint8 dhcp_parse_msg(DHCP_CLIENT *dhcp)
{
    ...
    {
        OS_DBG_ERR(DBG_MOD_NET, "DHCP bad op[%d] port [%d]", ripMsg.op, serPort);
        return DHCP_NONE;
    }
    ...
    if ((memcmp(ripMsg.chaddr, dhcp->mac, 6) != 0)
        || (ripMsg.xid != swapl(dhcp->xid)))
    {
        OS_DBG_ERR(DBG_MOD_NET, "DHCP bad mac[%02X.%02X.%02X.%02X.%02X.%02], xid[%08X]", 
    ...
    if (*((uint32*)(dhcp->serverIP)) != 0x00000000)
    {
        ...
        {
            OS_DBG_ERR(DBG_MOD_NET, "DHCP bad server[%d.%d.%d.%d]",
    ...
    while (p < endOfMsg) 
    {
        switch (*p++) 
        {
            ...
            case dhcpServerIdentifier:
            {
                ...
                {
                    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP find server [%d.%d.%d.%d]",
                                            serAddr[0], serAddr[1], serAddr[2], serAddr[3]);
                }
                break;
            }
...
static void dhcp_try_to_resend_msg(DHCP_CLIENT *dhcp)
{
    ...
    if (dhcp->retryCnt < DHCP_MAX_RETRY) 
    {
        ...
        if ((curTime - dhcp->lastTime) > DHCP_WAIT_TIMEOUT) 
        {
            ...
            OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP[%d]timeout, resend", dhcp->state);
        }
        else
        {
    ...
    else 
    {
        ...
        OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP[%d]retry cnt out, restart");
    }
}
...
static bool dhcp_leasedIP_valid(DHCP_CLIENT *dhcp)
{
    ...
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP SEND LEASED IP to %d.%d.%d.%d:%d",
    ...
    {
        OS_DBG_ERR(DBG_MOD_NET, "DHCP leased IP conflict, decline");
...
uint8 dhcp_client_process(DHCP_CLIENT *dhcp)
{
    ...
    if (dhcp->state != DHCP_STATE_INIT)
    {
        ...
        {
            OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP parse msg[%d]", msgType);
        }
    }
...
void dhcp_client_init(DHCP_CLIENT *dhcp)
{
    ...
    dhcp->xid = DEFAULT_XID + pb_util_random_num(DEFAULT_XID);
    dhcp->retryCnt = 0;
    dhcp->lastTime = 0;
    dhcp->leaseTime = 0;
    ...
    OS_DBG_TRACE(DBG_MOD_NET, DBG_INFO, "DHCP Init %08X", dhcp->xid);
}
